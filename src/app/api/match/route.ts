...
export async function GET(req: NextRequest) {
  ...
  // Get my profile ID
  const { data: myProfile } = await supabase.from('profiles').select('id').eq('clerk_user_id', userId).single();
  ...
  const [{ data: passes }, { data: sentRequests }, { data: blocks }] = await Promise.all([
    ...
    supabase.from('blocked_users').select('blocked_id, blocker_id').or(`blocker_id.eq.${myProfile.id},blocked_id.eq.${myProfile.id}`),
  ]);

  const excludedIds = [
    ...passes?.map((p: any) => p.to_user) ?? [],
    ...sentRequests?.map((r: any) => r.to_user) ?? [],
    ...(blocks ?? []).map((b: any) => (b.blocker_id === myProfile.id ? b.blocked_id : b.blocker_id)),
  ];
  ...
  if (excludedIds.length > 0) {
    // Safely pass exclusions as a quoted list to avoid malformed SQL generated by unquoted joins
...
export async function POST(req: NextRequest) {
  ...
  const { data: targetProfile } = await supabase.from('profiles').select('id, is_banned').eq('id', to_user_profile_id).single();
  ...
  const { data: reverseRequest } = await supabase.from('match_requests').select('id, status').eq('from_user', to_user_profile_id).eq('to_user', myProfile.id).single();
  if (reverseRequest && reverseRequest.status === 'pending') {
    // Mutual match — update to matched
    const { error: updateErr } = await supabase.from('match_requests').update({ status: 'matched' }).eq('id', reverseRequest.id);
    if (updateErr) return NextResponse.json({ error: updateErr.message }, { status: 500 });

    // Create conversation
    const { data: conv } = await supabase.from('conversations').insert({ user_a: myProfile.id, user_b: to_user_profile_id }).select().single();
  ...
  // Insert new request
  const { error } = await supabase.from('match_requests').insert({ from_user: myProfile.id, to_user: to_user_profile_id, status: 'pending' });
  if (error) {
  ...
  // Notify target
  await supabase.from('notifications').insert({ user_id: to_user_profile_id, type: 'match_request', related_user: myProfile.id });
