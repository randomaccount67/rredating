...
interface AdminReport {
  ...
  reviewed: boolean;
  created_at: string;
  reporter: { id: string; riot_id: string | null; riot_tag: string | null } | null;
  reported: { id: string; riot_id: string | null; riot_tag: string | null } | null;
}

function displayName(user: { riot_id: string | null; riot_tag: string | null } | null) {
...
export default function AdminPage() {
  ...
  const handleBan = async (profileId: string, ban: boolean) => {
    ...
    try {
      ...
      setUsers(prev => prev.map(u => u.id === profileId ? { ...u, is_banned: ban } : u));
      if (selectedUser?.id === profileId) setSelectedUser(prev => prev ? { ...prev, is_banned: ban } : prev);
    } catch {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8">
      {/* Header */}
      <div className="mb-8">
        <span className="font-mono text-[10px] tracking-widest text-[#FF4655] uppercase">// RESTRICTED ACCESS</span>
        <h1 className="font-extrabold text-4xl uppercase text-[#E8EAF0] mt-1"
        ...
        <div className="flex gap-4 mt-2 font-mono text-[10px] text-[#525566]">
          ...
          <span className={unreviewedReports.length > 0 ? 'text-[#FF4655]' : ''}>{unreviewedReports.length} UNREVIEWED</span>
        </div>
      </div>

      {error && (
        <div className="mb-4 bg-[#FF4655]/10 border border-[#FF4655]/30 px-4 py-2 font-mono text-xs text-[#FF4655]">{error}</div>
      )}

      {/* Unreviewed reports banner */}
      {unreviewedReports.length > 0 && (
        <div className="mb-6 bg-[#1A1D24] border border-[#FF4655]/30"
          ...
          <div className="px-5 py-3 border-b border-[#2A2D35] flex items-center gap-2">
            ...
            <span className="font-mono text-xs text-[#FF4655] uppercase tracking-wider">
              {unreviewedReports.length} Unreviewed Report{unreviewedReports.length !== 1 ? 's' : ''}
            </span>
          </div>
              ...
              <div key={r.id} className="px-5 py-3 flex items-center justify-between gap-4">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className="font-mono text-[10px] text-[#FF4655] border border-[#FF4655]/30 px-1.5 py-0.5">{r.reason}</span>
                    <span className="font-mono text-[10px] text-[#525566]">
                      {displayName(r.reporter)} → {displayName(r.reported)}
                    </span>
                  </div>
                  {r.details && (
                    <p className="text-[#525566] text-xs mt-1 truncate">{r.details}</p>
                  )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        {/* Users list */}
        <div className="lg:col-span-2 bg-[#1A1D24] border border-[#2A2D35]"
          style={{ clipPath: 'polygon(0 0, calc(100% - 12px) 0, 100% 12px, 100% 100%, 0 100%)' }}>
          <div className="px-4 py-3 border-b border-[#2A2D35] flex items-center gap-2">
            <Shield size={12} className="text-[#525566]" />
            <span className="font-mono text-[10px] uppercase tracking-wider text-[#8B8FA8]">All Users</span>
          </div>
          <div className="divide-y divide-[#2A2D35] max-h-[600px] overflow-y-auto">
            {users.map(u => {
              const name = u.riot_id ? `${u.riot_id}#${u.riot_tag}` : 'NO PROFILE';
              const reportCount = reports.filter(r => r.reported_id === u.id).length;
              const unreviewedCount = reports.filter(r => r.reported_id === u.id && !r.reviewed).length;
              return (
                <button
                  key={u.id}
                ...
                >
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      <span className="font-mono text-xs text-[#E8EAF0] truncate">{name}</span>
                      {u.is_banned && <span className="font-mono text-[8px] text-[#FF4655] border border-[#FF4655]/30 px-1">BANNED</span>}
                      {u.is_admin && <span className="font-mono text-[8px] text-amber-400 border border-amber-400/30 px-1">ADMIN</span>}
                    </div>
                    <div className="flex items-center gap-2 mt-0.5">
                      {u.region && <span className="font-mono text-[9px] text-[#525566]">{u.region}</span>}
                      <span className="font-mono text-[9px] text-[#525566]">
                        {new Date(u.created_at).toLocaleDateString()}
                      </span>
                      {reportCount > 0 && (
                        <span className={`font-mono text-[9px] ${unreviewedCount > 0 ? 'text-[#FF4655]' : 'text-[#525566]'}`}>
                          <Flag size={8} className="inline mr-0.5" />{reportCount}
                        </span>
                      )}
                    </div>
                  </div>
                  <ChevronRight size={12} className="text-[#525566] flex-shrink-0" />
                </button>
              );
            })}
          </div>
        ...
        <div className="lg:col-span-3">
          {!selectedUser ? (
            <div className="bg-[#1A1D24] border border-[#2A2D35] h-full min-h-[300px] flex items-center justify-center"
              style={{ clipPath: 'polygon(0 0, calc(100% - 12px) 0, 100% 12px, 100% 100%, 0 100%)' }}>
              <p className="font-mono text-[10px] text-[#525566] uppercase">Select a user to view details</p>
            </div>
          ) : (
            <div className="space-y-4">
              ...
              <div className="bg-[#1A1D24] border border-[#2A2D35]"
                ...
                <div className="px-5 py-4 border-b border-[#2A2D35] flex items-center justify-between">
                  <div>
                    <div className="flex items-center gap-2">
                      <h2 className="font-mono text-sm text-[#E8EAF0]">
                        {selectedUser.riot_id ? `${selectedUser.riot_id}#${selectedUser.riot_tag}` : 'NO PROFILE'}
                      </h2>
                      {selectedUser.is_banned && (
                        <span className="font-mono text-[9px] text-[#FF4655] border border-[#FF4655]/30 px-1.5 py-0.5">BANNED</span>
                      )}
                    </div>
                    <p className="font-mono text-[9px] text-[#525566] mt-0.5">
                      ID: {selectedUser.id}
                  ...
                  >
                    <Ban size={13} />
                    {actionLoading === selectedUser.id ? '...' : selectedUser.is_banned ? 'UNBAN USER' : 'BAN USER'}
                  </button>
                </div>
                ...
                <div className="px-5 py-4 grid grid-cols-2 gap-3 text-xs border-b border-[#2A2D35]">
                  {[
                    ...
                    <div key={label as string}>
                      <span className="label text-[9px] block">{label as string}</span>
                      <span className="font-mono text-[11px] text-[#8B8FA8]">{value as string}</span>
                    </div>
                  ) : null)}
                </div>
                ...
                {selectedUser.about && (
                  <div className="px-5 py-3">
                    ...
                    <p className="text-[#8B8FA8] text-xs leading-relaxed">{selectedUser.about}</p>
                  </div>
                )}
              </div>
              ...
              <div className="bg-[#1A1D24] border border-[#2A2D35]"
                ...
                {userReports.length === 0 ? (
                  <div className="px-4 py-6 text-center">
                    <p className="font-mono text-[10px] text-[#525566]">No reports filed against this user.</p>
                  </div>
                ) : (
                  <div className="divide-y divide-[#2A2D35]">
                    {userReports.map(r => (
                      <div key={r.id} className="px-4 py-3 flex items-start justify-between gap-4">
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center gap-2 flex-wrap">
                            <span className={`font-mono text-[9px] border px-1.5 py-0.5 ${r.reviewed ? 'border-[#2A2D35] text-[#525566]' : 'border-[#FF4655]/30 text-[#FF4655]'}`}>
                              {r.reason}
                            </span>
                            {r.reviewed && <span className="font-mono text-[9px] text-green-500">✓ REVIEWED</span>}
                          </div>
                          <p className="font-mono text-[9px] text-[#525566] mt-1">
                            By: {displayName(r.reporter)} · {new Date(r.created_at).toLocaleDateString()}
                          </p>
                          {r.details && (
