  ...
  return (
    <>
      <header className="sticky top-0 z-50 border-b border-[#252830] bg-[#0B0D11]/95 backdrop-blur-sm">
        ...
        {/* Main nav */}
        <nav className="max-w-7xl mx-auto px-4 h-14 flex items-center justify-between" aria-label="Main navigation">
          {/* Logo */}
          <Link href="/" className="flex items-center gap-2 group flex-shrink-0" aria-label="RRedating home">
          ...
          <div className="hidden md:flex items-center justify-end gap-2">
            <button
              ...
              className="flex items-center gap-1.5 px-2.5 py-1.5 text-[10px] font-bold uppercase tracking-wider border border-[#5865F2]/40 text-[#5865F2] hover:bg-[#5865F2]/10 hover:border-[#5865F2] transition-all"
              style={{ fontFamily: 'Barlow Condensed, sans-serif' }}
            >
            ...
            {isSignedIn && (
              <Link
                href="/settings"
                aria-label="Settings"
                className={`flex items-center justify-center p-2 text-[10px] font-bold uppercase tracking-wider border transition-all
            ...
            {isSignedIn ? (
              <button
                ...
                aria-label="Sign out"
                className="flex items-center gap-1.5 px-2.5 py-1.5 text-[10px] font-bold uppercase tracking-wider border border-[#2A2D35] text-[#8B90A8] hover:text-[#ECF0F8] hover:border-[#525566] transition-all"
                style={{ fontFamily: 'Barlow Condensed, sans-serif' }}
              >
          ...
          {/* Mobile right side: notification badge + hamburger */}
          <div className="flex md:hidden items-center gap-2">
            {isSignedIn && (
              <Link
                href="/notifications"
                aria-label={`Notifications${unreadNotifs > 0 ? ` (${unreadNotifs} unread)` : ''}`}
                className="relative p-2 text-[#8B90A8] hover:text-[#ECF0F8] transition-colors"
              >
            ...
            <button
              ...
              className="p-2 text-[#8B90A8] hover:text-[#ECF0F8] transition-colors"
            >
        ...
        {mobileMenuOpen && (
          <div className="md:hidden border-t border-[#252830] bg-[#0B0D11]">
            <div className="flex flex-col divide-y divide-[#1A1D24]">
              {isSignedIn && isAdmin && (
                <Link
                  href="/admin"
                  onClick={() => setMobileMenuOpen(false)}
                  className={`flex items-center gap-3 px-5 py-3.5 text-sm font-bold tracking-wider uppercase transition-colors
                    ${pathname.startsWith('/admin') ? 'text-[#FF4655]' : 'text-amber-400'}`}
                  style={{ fontFamily: 'Barlow Condensed, sans-serif' }}
                >
              ...
              {navLinks.map((link) => {
                ...
                return (
                  <Link
                    ...
                    href={link.href}
                    onClick={() => setMobileMenuOpen(false)}
                    className={`relative flex items-center gap-3 px-5 py-3.5 text-sm font-bold tracking-wider uppercase transition-colors
                      ${active ? 'text-[#FF4655]' : 'text-[#8B90A8]'}`}
                    style={{ fontFamily: 'Barlow Condensed, sans-serif' }}
                  >
                    {link.icon}
                    {link.label}
              ...
              <button
                onClick={() => { setMobileMenuOpen(false); setComplainOpen(true); }}
                className="flex items-center gap-3 px-5 py-3.5 text-sm font-bold tracking-wider uppercase text-[#8B90A8] transition-colors text-left"
                style={{ fontFamily: 'Barlow Condensed, sans-serif' }}
              >
              ...
              <button
                onClick={() => { setMobileMenuOpen(false); setDiscordOpen(true); }}
                className="flex items-center gap-3 px-5 py-3.5 text-sm font-bold tracking-wider uppercase text-[#5865F2] transition-colors text-left"
                style={{ fontFamily: 'Barlow Condensed, sans-serif' }}
              >
              ...
              {isSignedIn && (
                <Link
                  ...
                  onClick={() => setMobileMenuOpen(false)}
                  className={`flex items-center gap-3 px-5 py-3.5 text-sm font-bold tracking-wider uppercase transition-colors
                    ${pathname.startsWith('/settings') ? 'text-[#FF4655]' : 'text-[#8B90A8]'}`}
                  style={{ fontFamily: 'Barlow Condensed, sans-serif' }}
                >
              ...
              {isSignedIn ? (
                <button
                  ...
                  className="flex items-center gap-3 px-5 py-3.5 text-sm font-bold tracking-wider uppercase text-[#8B90A8] hover:text-[#FF4655] transition-colors text-left"
                  style={{ fontFamily: 'Barlow Condensed, sans-serif' }}
                >
                ...
                <div className="flex gap-3 px-5 py-3.5">
                  <Link href="/sign-in" onClick={() => setMobileMenuOpen(false)} className="btn-ghost text-sm py-2 px-4 flex-1 text-center">SIGN IN</Link>
                  <Link href="/sign-up" onClick={() => setMobileMenuOpen(false)} className="btn-primary text-sm py-2 px-4 flex-1 text-center">JOIN UP</Link>
                </div>
              )}
            </div>
          </div>
